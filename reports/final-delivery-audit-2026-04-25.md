...
- `./scripts/release-verify.sh http://localhost`:13 passed,0 failed。Manifest:`reports/release/release-manifest-2026-04-25-124450.md`。
- `python3 -m pytest -q`:246 passed,13 skipped,1 warning。
- `npm run build` in `frontend/launcher`:passed。
- `npm test -- --reporter=line` in `tests/e2e`:34 passed,2 skipped。
...
- DMG SHA-256:`17df0b4013f80e2a3e869d0de350bfc7e0e8414d0c6fcd4c2d2c4ac7c06c214f`。
...
| # | 項目 | 對應 Finding | 估時 |
|---|---|---|---|
| 1 | Today composer 加附件拖放區 + file ribbon | F-03 | 0.5d |
| 2 | Sidebar 收斂(刪重複項 / 隱「智慧引擎」/ 4 項收進 Workspace) | F-01 / F-04 / F-06 | 0.5d |
| 3 | 首頁 hero H2 與 sidecar 文案修(「下一棒/棒」改一般用語) | F-10 / F-11 | 0.2d |
| 4 | 乾淨 Mac VM 跑完整 DMG 安裝 + smoke · 結果寫進 release manifest | F-08 | 1d |
| 5 | LibreChat RAG 上傳 + 引用實測 | F-24 | 0.3d |
| 6 | 補一條 PM happy-path E2E(composer → 上傳 → 存工作包 → 填 handoff → 複製) | §7.3#1 | 0.5d |
