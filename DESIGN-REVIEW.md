...
**Sterio 決策需求**:本公司老闆買 16GB 還是 24GB?價差 NT$ 7,000,但能避免明年升級風險。
...
**發現**:MongoDB 7.0+ 採 SSPL 授權。商業場景(本公司)使用**一般不影響**,但 Docker 官方鏡像法律風險較低。DeepChat、FerretDB 是開源替代。

**現狀評估**:LibreChat 官方就是用 MongoDB,本公司 10 人內部使用無商業轉售,**視為低風險**,但 `docs/05-SECURITY.md` 要記錄此事。
...
**發現**:Docker Desktop 2022 起對>250 人公司收費。本公司 10 人免費,但**授權條款可能變更**。

**修正**:
- 在 `.env.example` 註明 Docker Desktop 授權適用於本公司規模
...
- 容器啟動時透過 shell script 從 Keychain 讀取並注入環境變數:
  ```bash
  export ANTHROPIC_API_KEY=$(security find-generic-password -s 'company-ai-anthropic' -w)
  docker compose up -d
  ```
...
### 🟡 問題 S-3:資料跨境傳輸合規
**發現**:Level 02 資料(招標須知、建議書)送 Anthropic US 伺服器。本公司處理政府案件,**可能涉及個資法(PDPA)與政府機敏**。

**修正**:
- 建「資料傳輸告知書」模板放 `docs/05-SECURITY.md`
- 跟本公司老闆討論:是否需與客戶取得同意(尤其政府標案)
...
**對本公司的價值**:Week 4 時,**核心產能 Agent 全員可用**,同仁已開始節省時間。後續優化不影響主要效益。
...
**實際 5-7 hr/月**,時薪換算 NT$ 700-1000,遠低於本公司報價合理值。
...
### 🟢 問題 P-5:試用期風險保護不足
**發現**:「3 個月免費試用」後若本公司不續約,系統、知識庫、Agent 全部是他們的,可自行用下去。

**修正**:在合約明確:
- 系統(硬體 + 軟體部署)為本公司所有 ← 這是賣點不改
- **但維運服務不同**:無維運合約後,LibreChat 升級、Anthropic API 問題、Agent 調優都沒人管
- 明文列出「無維運的風險」讓本公司知情決策
...
- Word 模板用本公司 CIS(logo、字型、頁首頁尾)
- **Week 3 新增任務**:設計 3 個本公司品牌 Word 模板供 export 時選用
...
- `tasks/week-3` 新增「本公司法務初步確認」check point
...
這樣本公司 Week 4 就有實際產能提升,而不是承諾了 29 個功能但品質都堪堪及格。
**寧可少做做透,勝過多做做爛**。
