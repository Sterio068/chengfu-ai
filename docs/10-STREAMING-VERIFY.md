...
docker exec -it company-ai-librechat \
    curl -N -X POST -H "Authorization: Bearer <JWT>" \
         ...
         -d '{"agent_id":"<AGENT_ID>","conversationId":"new","parentMessageId":"00000000-0000-0000-0000-000000000000","text":"寫一首詩","endpoint":"agents","isContinued":false,"isTemporary":false,"messageId":"stream-test-001"}'
# 預期:文字逐字流出(不是等完才印)
```

### 3. curl 透過 nginx
```bash
curl -N -X POST http://localhost/api/agents/chat \
     -H "Authorization: Bearer <JWT>" \
     -H "Content-Type: application/json" \
     -d '{"agent_id":"<AGENT_ID>","conversationId":"new","parentMessageId":"00000000-0000-0000-0000-000000000000","text":"寫一首詩","endpoint":"agents","isContinued":false,"isTemporary":false,"messageId":"stream-test-002"}'
