# NAS 接入規格(老闆明確:80% 檔案在 NAS · 不只 NAS)

> **🔥 2026-04-21 兩段更新:**
> 1. 老闆答 Q3:scope 從「讀取」→「整個 NAS 完整整合」(讀所有類型 · 分專案資料夾)
> 2. 老闆補充:「**不只 NAS · 要可以指定專案資料夾**」→ 升為**多來源知識庫機制**
>
> **完整實作規格在:** `docs/V1.1-IMPLEMENTATION-SPEC.md` §E(39-52h · Week 7-10 v1.2)
> 本檔只是歷史定位文件 · 真實作以 V1.1-SPEC §E 為準
...
| **NAS 有沒有每日備份 · 誰負責** | 企業 AI 不擴增 NAS 備份責任 | 企業 AI 等於背鍋 |
...
```bash
# macOS Mac mini 上
mkdir -p /Volumes/CompanyAI-NAS
mount -t smbfs //<ai-ro-user>:<pwd>@<nas-ip>/<share> /Volumes/CompanyAI-NAS
# 自動掛載:寫進 /etc/auto_master · 開機自動 mount
```

**帳密存哪:**
- Keychain(與其他機敏同位置):`company-ai-nas-user` / `company-ai-nas-password`
...
**首批索引範圍**(本公司答完 5 項才填):
```
/Volumes/CompanyAI-NAS/projects/**/*.docx    <- 建議書
/Volumes/CompanyAI-NAS/projects/**/*.pdf     <- 招標附件
/Volumes/CompanyAI-NAS/kv/**/*.ai            <- KV 原稿(讀不了,只記 metadata)
/Volumes/CompanyAI-NAS/photos/**/*.jpg       <- 活動照片(之後做 CLIP vision 索引)
```

**索引 schema** (`backend/accounting/nas_index.py` 會做 cron):
```python
{
    "path": "/Volumes/CompanyAI-NAS/projects/2024-淨灘案/建議書v3.docx",
    "filename": "建議書v3.docx",
    "project": "2024-淨灘案",
    "content_preview": "本公司...(前 2000 字)",
    "size": 1284392,
...
def nas_read(path: str):
    ...
    · 只允許 /Volumes/CompanyAI-NAS 底下的路徑
...
```json
{
  "info": { "title": "本公司 NAS 檔案讀取", "version": "v1" },
  "paths": {
    "/nas/search": {
      "get": {
        "summary": "搜 NAS 全文",
...
| 項目 | 樂觀 | 悲觀 | 備註 |
|---|---|---|---|
| 確認 5 項 + 建 ro 帳號 | 0h(本公司) | 2h | 要本公司做 |
...
- **NAS 備份** — NAS 自己要備份,企業 AI 不擔這個
- **索引所有檔** — 只索引本公司答案指定的資料夾
...
- [ ] 本公司 IT 確認「AI 掛 NAS」不違反內部 policy
