# docs/DECISIONS.md — 企業 AI 系統決策紀錄
...
- **前端策略**:Launcher 使用 vanilla ES modules,不改成 React / Next.js / Tailwind；正式前端可見文字不綁定「本公司」,以多公司可用白標化為方向；LibreChat 保持上游可升級。
...
- **理由**:本公司同仁需要穩定、可教學、可驗收的工作入口；10 核心 Agent 已涵蓋 5 Workspace 的主要閉環,比 29 個平鋪 Agent 更適合 AI 小白。
...
- **決策**:本公司正式部署以 OpenAI 作為預設主力 AI；Launcher 提供前端切換 OpenAI / Claude。Anthropic 不再是唯一必要條件,但可作為備援。
- **理由**:使用者期待接近「正常 AI 使用」的穩定體驗；OpenAI 作為預設可降低上手摩擦,Claude 保留長文件與既有 prompt 工作流彈性。
- **驗收**:Keychain 至少有 `company-ai-openai-key`；`scripts/create-agents.py --provider both` 可建立雙引擎 Agent；前端切換後新對話優先命中對應 provider。
...
- **理由**:本公司工作涉及客戶、成本、標案與對外發聲,需保留人工關卡。
...
- **決策**:`company_ai_permissions` 不一次切成全量 RBAC,而是先從高風險入口逐步接上 `require_permission_dep()`。
...
- **全域規則**:停用帳號(`company_ai_active=false`)即使在 `ADMIN_EMAILS` 或持有 permission,也不得進一般或 admin endpoint。
...
- **決策**:正式使用者 UI、安裝提示與教學入口不應出現固定「本公司」品牌字樣；採「智慧助理 / AI 工作台 / 本公司」等可多公司部署的泛用命名。
- **允許保留**:repo 名稱、歷史文件、舊報告、DMG 檔名與內部註解可暫留 CompanyAIWorkspace/本公司,避免一次大規模改名造成交付風險。
...
| 優先級 | 決議項 | 目前建議 |
|---|---|---|
| P0 | Cloudflare Tunnel 正式 domain | 需本公司提供最終 `ai.<domain>` 與 Access policy 名單 |
...
| P1 | LINE workflow 是否進 vNext | 若本公司主要資料入口是 LINE,建議排在 Chrome Extension 後 |
