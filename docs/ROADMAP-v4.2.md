...
- [ ] `docs/REMOTE-DEV.md` 外包文件:
  ...
  - 權限範圍:只碰 `/Users/sterio/Workspace/CompanyAIWorkspace/*`
...
├── 測試 + 文件(3-4h)
└── 本公司同仁整理專案資料夾(Champion 分週 · 不阻擋 Sterio)
...
3. **遠端開發權限範圍** — 外包工程師只能改 `CompanyAIWorkspace/` 還是整個 Mac mini?
...
- **修法(R8 codex 推薦最簡方向):**
  1. nginx strip inbound `X-Agent-Num`(在 company-ai-proxy.conf 加 `proxy_set_header X-Agent-Num "";`)
