...
預期:`✓ 12` 接上。對應:
- 🎯 投標顧問 + ✨ 主管家 → PCC 標案查詢(g0v · 無需 key)
- 💰 財務試算 + ✨ 主管家 → 內建會計 API(X-Internal-Token)
- 🎨 設計夥伴 + ✨ 主管家 → **OpenAI gpt-image-1 / ChatGPT Images 2.0** 生圖
  · 用既有 OPENAI_API_KEY · 不用業主再開 Fal.ai 帳戶
  · 支援文字生圖 + image-to-image 改稿 + inpaint mask
