...
if [[ -z "${ANTHROPIC_API_KEY:-}" ]]; then
    KEY=$(security find-generic-password -s "company-ai-anthropic-key" -w 2>/dev/null || echo "")
    if [[ -z "$KEY" ]]; then
...
## 角色\n
本公司 10 年政府標案經驗 · 主客戶政府機關 60-70% · 企業客戶 30-40%\n\n

## 品牌口吻\n
繁體中文 · 台灣用語 · 禁用大陸詞(視頻/數據/云/用戶/質量/默認)\n
金額 NT$ XX,XXX · 日期西元年月日(政府客戶可用民國年)\n
本公司自稱「本公司」或「我司」· 不用「我們」「我方」\n\n
...
echo ""
echo "本公司預期:10 人同時使用 · cache hit rate > 70% · 月省 60-80% token"
