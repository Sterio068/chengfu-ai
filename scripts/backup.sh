...
# 設定 cron(每日 02:00):
#   crontab -e
#   0 2 * * * /Users/<user>/Workspace/CompanyAIWorkspace/scripts/backup.sh >> /var/log/company-ai-backup.log 2>&1

set -euo pipefail

BACKUP_ROOT="${HOME}/company-ai-backups"
...
if command -v gpg > /dev/null 2>&1 && gpg --list-keys "$GPG_RECIPIENT" > /dev/null 2>&1; then
    ...
    if [[ "$ALLOW_PLAINTEXT_BACKUP" != "1" ]]; then
        echo "❌ 找不到 GPG key '${GPG_RECIPIENT}' · 正式備份不允許落明文" >&2
        echo "   先執行:gpg --full-generate-key  (name 設為 ${GPG_RECIPIENT})" >&2
...
ARCHIVE="${DAILY_DIR}/company-ai-${DATE}.archive.gz"
if [[ "$GPG_AVAILABLE" == "1" ]]; then
    # GPG 可用 · 一條 pipeline · 磁碟上永遠不落明文
    ARCHIVE="${DAILY_DIR}/company-ai-${DATE}.archive.gz.gpg"
    docker exec company-ai-mongo mongodump --archive --db "$MONGO_DB_NAME" --quiet 2>/dev/null \
        | gzip -9 \
        | gpg --batch --yes --encrypt --recipient "$GPG_RECIPIENT" --output "$ARCHIVE"
...
MEILI_DUMP="${DAILY_DIR}/company-ai-meili-${DATE}.tar.gz"
...
if [[ -n "$MEILI_KEY" ]]; then
    ...
    DUMP_RESP=$(docker exec company-ai-meili wget -qO- \
    ...
    if [[ -n "$TASK_UID" && "$TASK_UID" != "null" ]]; then
        ...
        for i in $(seq 1 24); do
            TASK_JSON=$(docker exec company-ai-meili wget -qO- \
                --header="Authorization: Bearer ${MEILI_KEY}" \
        ...
        if [[ "$MEILI_STATUS" == "succeeded" ]]; then
            ...
            if [[ -d "${PROJECT_DIR}/config-templates/data/meili/dumps" ]]; then
                ...
                TASK_JSON2=$(docker exec company-ai-meili wget -qO- \
                    --header="Authorization: Bearer ${MEILI_KEY}" \
                    http://127.0.0.1:7700/tasks/${TASK_UID} 2>/dev/null || echo "")
                ...
                if [[ -n "$DUMP_UID" && -f "$DUMP_FILE_PATTERN" ]]; then
                    if [[ "$GPG_AVAILABLE" == "1" ]]; then
                        MEILI_DUMP="${DAILY_DIR}/company-ai-meili-${DATE}.dump.gpg"
                        ...
                        MEILI_DUMP="${DAILY_DIR}/company-ai-meili-${DATE}.dump"
...
if [[ "$GPG_AVAILABLE" == "1" ]]; then
    KB_ARCHIVE="${DAILY_DIR}/company-ai-kb-${DATE}.tar.gz.gpg"
    tar czf - -C "$PROJECT_DIR" \
        ...
        frontend/nginx/ \
        2>/dev/null \
        | gpg --batch --yes --encrypt --recipient "$GPG_RECIPIENT" --output "$KB_ARCHIVE"
    echo "  🔐 KB pipeline 直接加密: $KB_ARCHIVE"
else
    KB_ARCHIVE="${DAILY_DIR}/company-ai-kb-${DATE}.tar.gz"
    tar czf "$KB_ARCHIVE" \
        -C "$PROJECT_DIR" \
        knowledge-base/ \
        config-templates/librechat.yaml \
...
KEYCHAIN_LIST="${DAILY_DIR}/company-ai-keychain-inventory-${DATE}.txt"
security dump-keychain 2>/dev/null | grep -oE 'company-ai-[a-z-]+' | sort -u > "$KEYCHAIN_LIST" 2>/dev/null || true
...
if [[ "$GPG_AVAILABLE" == "1" ]]; then
    ...
    echo "  ⚠ 未設定 GPG key '${GPG_RECIPIENT}' · 所有備份明文 · 異機上傳會被跳過"
...
#   gpg --full-generate-key   (name 設為 'company-ai')
#   rclone config              (遠端名 "company-ai-offsite",可 Backblaze B2 / Cloudflare R2 / S3)
...
if command -v rclone > /dev/null 2>&1 && rclone listremotes 2>/dev/null | grep -q "^${OFFSITE_REMOTE%%:*}:"; then
    ...
    if [[ "$ARCHIVE" == *.gpg ]]; then
        ...
        echo "  ⚠ 未 GPG 加密 · 不上傳異機 · 先設 '${GPG_RECIPIENT}' GPG key"
    fi
