...
if [[ -z "${LIBRECHAT_ADMIN_EMAIL:-}" && "$(uname -s)" == "Darwin" ]]; then
    LIBRECHAT_ADMIN_EMAIL="$(security find-generic-password -s company-ai-admin-install-email -w 2>/dev/null || true)"
fi
if [[ -z "${LIBRECHAT_ADMIN_PASSWORD:-}" && "$(uname -s)" == "Darwin" ]]; then
    LIBRECHAT_ADMIN_PASSWORD="$(security find-generic-password -s company-ai-admin-install-password -w 2>/dev/null || true)"
fi
...
echo ""
echo "[容器狀態]"
check "nginx 容器運行中"        "docker ps --filter name=company-ai-nginx --filter status=running -q | grep -q ."
check "LibreChat 容器運行中" "docker ps --filter name=company-ai-librechat --filter status=running -q | grep -q ."
check "MongoDB 容器運行中"   "docker ps --filter name=company-ai-mongo --filter status=running -q | grep -q ."
check "Meilisearch 容器運行中" "docker ps --filter name=company-ai-meili --filter status=running -q | grep -q ."
check "Accounting 容器運行中" "docker ps --filter name=company-ai-accounting --filter status=running -q | grep -q ."
check "Uptime Kuma 容器運行中" "docker ps --filter name=company-ai-uptime --filter status=running -q | grep -q ."
...
check "偽造 X-User-Email 不可通過 admin API" "curl -s -o /dev/null -w '%{http_code}' -H 'X-User-Email: ${LIBRECHAT_ADMIN_EMAIL:-admin@company-ai.local}' '${BASE_URL}/api-accounting/admin/users' | grep -Eq '^(401|403)$'"
check "RAG adapter 不對外暴露" "curl -s -o /dev/null -w '%{http_code}' '${BASE_URL}/api-accounting/rag/health' | grep -Eq '^(403|404)$'"

echo ""
echo "[資源佔用]"
LIBRE_MEM=$(docker stats --no-stream --format "{{.MemUsage}}" company-ai-librechat 2>/dev/null | awk '{print $1}')
echo "  LibreChat 記憶體:$LIBRE_MEM"
MONGO_MEM=$(docker stats --no-stream --format "{{.MemUsage}}" company-ai-mongo 2>/dev/null | awk '{print $1}')
...
echo ""
echo "[備份]"
LATEST_BACKUP=$(find "${HOME}/company-ai-backups/daily" -type f -name "company-ai-*" 2>/dev/null | sort | tail -1)
