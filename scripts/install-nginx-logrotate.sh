#!/bin/bash
# ============================================================
# [DEPRECATED] 企業 AI · nginx log rotation(host macOS)
# ============================================================
...
# 產出 /etc/newsyslog.d/company-ai.conf · 每日 rotate · 保留 30 天
...
#
# 驗證:
#   sudo newsyslog -nvv | grep company_ai
# ============================================================
...
NEWSYSLOG_CONF="/etc/newsyslog.d/company-ai.conf"
...
# 企業 AI · nginx access log · 每日 00:00 rotate · 保留 30 份 · gzip
$LOG_DIR/company_ai.access.log    $USER:staff   644  30    *    @T00  Z
$LOG_DIR/company_ai.error.log     $USER:staff   644  30    *    @T00  Z
EOF

echo "✅ 已寫 $NEWSYSLOG_CONF"
echo ""
echo "[驗證]:"
sudo newsyslog -nvv | grep -A2 company_ai || echo "  (尚未跑 · 00:00 自動 rotate)"
echo ""
echo "[手動測試(不等到午夜)]:"
