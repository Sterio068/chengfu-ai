...
def list_leads(
    ...
    if owner and not _is_admin_user(email) and owner != email:
        raise HTTPException(403, "只能查看自己的商機")
    if owner:
        q["owner"] = owner
    elif not _is_admin_user(email):
        q["owner"] = email
...
def create_lead(lead: Lead, email: str = require_user_dep()):
    from main import db
...
def update_lead(lead_id: str, updates: dict, email: str = require_user_dep()):
    ...
    old = db.crm_leads.find_one({"_id": oid}, {"stage": 1, "owner": 1})
    if not old:
        raise HTTPException(404, "lead 不存在")
    if not _is_admin_user(email) and old.get("owner") != email:
        raise HTTPException(403, "只能更新自己負責的商機")
...
def delete_lead(lead_id: str, email: str = require_user_dep()):
    ...
    if not _is_admin_user(email):
        q["owner"] = email
    r = db.crm_leads.delete_one(q)
    if r.deleted_count == 0:
        if db.crm_leads.find_one({"_id": oid}, {"_id": 1}):
            raise HTTPException(403, "只能刪除自己負責的商機")
        raise HTTPException(404, "lead 不存在")
...
def add_lead_note(
    ...
    if r.matched_count == 0:
        if db.crm_leads.find_one({"_id": _lead_oid(lead_id)}, {"_id": 1}):
            raise HTTPException(403, "只能更新自己負責的商機")
        raise HTTPException(404, "lead 不存在")
    return {"added": True}
