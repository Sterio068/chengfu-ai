...
3. **🔴 投資報酬的 baseline 未建立** — 6 個月後本公司問「省了多少」時無從答起
...
- 本公司老闆如果 6 個月後問:「這東西真的值嗎?」,Sterio 只能說「感覺變快了」

**修正**:Week 0 新增「baseline 量測週」,請本公司各角色填寫:
...
**缺口**:提案沒有「SaaS vs 本地部署」實戰比較表。業務在講單一數字,本公司老闆要的是決策對照。

**修正**:新增 `docs/07-WHY-NOT-SAAS.md`:

| 面向 | ChatGPT Team | 企業 AI 系統 |
|---|---|---|
...
| **本公司 CIS** | 無 | 新聞稿等產出自動套本公司風格 |
| **既有資料整合** | 要每次上傳 | 知識庫自動檢索 |
| **離線備用** | 完全無 | 階段二可加 Ollama |
| **政府標案敏感度** | 風險高 | 可分級管理 |

真正的論述:「前兩年 ChatGPT 便宜,但本公司是公關業,**資料主權 + 客製化**價值遠超 NT$ 50K/年 差距」。
...
**修正**:mockup 改成「本月投標 ____ 次」「勝率 ____」等空白欄位,讓本公司自己填數據。不要預設數字,尤其是勝率這種難以歸因給 AI 的指標。
...
**對本公司意義**:若啟用自動更新,某天早上開不起來是常態,不是例外。
...
- 若本公司有大量早年掃描標案 PDF,OCR 錯誤會讓 RAG 查詢錯亂
...
- **Filesystem MCP** 讓 AI 能讀本公司 NAS / 雲端硬碟的所有檔案
...
- **Phase 3**(v1.2):Slack / Line MCP(若本公司有用)
- 每個 MCP 建置前,評估 OAuth 安全性與本公司 IT 許可
...
- 明確寫入本公司內部:Champion 每週有 4 小時「AI 支援時間」,不算其他工作超時
...
- 本公司同仁不會自己調 prompt,這事落在 Sterio 身上
...
**現實**:若 Sterio 因故無法服務(生病、換職涯、不合作),本公司:
- 系統還在跑,但遇到問題沒人修
- Agent 不能優化,逐漸過時
- LibreChat 升級無人處理
...
### 🟡 問題 M-1:本公司若要換平台,資料能拿走嗎?
**現實**:本公司 3 年後可能想換到 OpenAI Enterprise 或其他平台。
...
**現實檢視 tasks/**:
- Week 1 說「請本公司提供固定 IP」—— 若本公司 IT 人員不配合怎麼辦?
- Week 2 說「收集 10 人 email 建帳號」—— email 格式本公司還沒決定?
- Week 3 說「灌入本公司知識庫」—— 本公司要提供哪些檔案?還沒盤點

**修正**:新建 `tasks/week-0-prep.md`(動工前本公司需確認清單):
...
## 帳號與域名
- [ ] 本公司 email 格式:name@company_ai.com.tw?
- [ ] 10 位同仁正式名單與 email
- [ ] 域名 ai.company_ai.com.tw 是否已在 Cloudflare?若無需註冊

## 資料準備(Week 3 會用)
- [ ] 預計灌入知識庫的檔案清單(多少份?大約多大?)
- [ ] 本公司 CIS 手冊(logo、色票、字型規範)
- [ ] 本公司過往優質新聞稿、建議書樣本

## 合約與合規
- [ ] 員工 PDPA 告知書(由本公司法務準備)
- [ ] 系統使用守則
- [ ] 本公司法律顧問是否需審閱?
...
| K-1 | 動工前本公司需確認項未列 | Week 1 會卡住 | 新增 tasks/week-0-prep.md |
...
1. 🔴 與本公司確認 `tasks/week-0-prep.md` 所有事項
